        ...
        $scope.optimalPriorityAssignment = function(threads) {
            ...
            for(var K=1; K<N; K++) {
                for(var Next=K; Next<N; Next++) {
                    ...
                    //$scope.calculateSimpleRTA(threads);
                    //$scope.calculateComplexRTA(threads);

                    var ok = $scope.isFeasible(threads[K]);
                    console.log('isFeasible', ok, K, threads[K]);
                    if(ok) break;

                }
                if(!ok) {
                    console.error('no RTA found');
                    return false;
                }
            }
        }
